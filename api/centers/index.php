...
switch ("$method:$action") {
    ...
    case 'GET:': {
        ...
        if (!empty($_GET['type'])) {
            ...
            if (in_array($_GET['type'], $allowed, true)) {
                $where[] = 'rc.worship_type = ?'; $params[] = $_GET['type'];
            }
        }
        if (!empty($_GET['q'])) {
            $where[] = '(rc.name LIKE ? OR rc.address LIKE ?)';
            $q = '%' . $_GET['q'] . '%'; $params[] = $q; $params[] = $q;
        }
        ...
        $stmt = $pdo->prepare("
            ...
            FROM religious_centers rc WHERE $whereSQL ORDER BY rc.name
        ");
        $stmt->execute($params);
        $rows = $stmt->fetchAll();
        ...
        foreach ($rows as &$r) {
            ...
            $r['radius'] = (int)$r['radius']; $r['household_count'] = (int)$r['household_count'];
    ...
    case 'GET:nearby': {
        ...
        $pdo  = Database::get();
        $stmt = $pdo->prepare("
            SELECT rc.*, (6371 * ACOS(COS(RADIANS(?)) * COS(RADIANS(rc.latitude)) * COS(RADIANS(rc.longitude) - RADIANS(?)) + SIN(RADIANS(?)) * SIN(RADIANS(rc.latitude)))) AS distance_km
            FROM religious_centers rc WHERE rc.is_active = 1 HAVING distance_km <= ? ORDER BY distance_km LIMIT 10
        ");
        $stmt->execute([$lat, $lng, $lat, $km]);
        $rows = $stmt->fetchAll();
        foreach ($rows as &$r) { $r['latitude'] = (float)$r['latitude']; $r['longitude'] = (float)$r['longitude']; $r['distance_km'] = round((float)$r['distance_km'], 3); }
        unset($r);
    ...
    case 'GET:coverage': {
        ...
        $stmt = $pdo->prepare("
            SELECT sub.* FROM (
                SELECT h.id, h.head_name, h.latitude, h.longitude, h.poverty_status, h.aid_status, h.dependents,
                    (6371000 * ACOS(COS(RADIANS(:lat1)) * COS(RADIANS(h.latitude)) * COS(RADIANS(h.longitude) - RADIANS(:lng)) + SIN(RADIANS(:lat2)) * SIN(RADIANS(h.latitude)))) AS distance_m
                FROM households h WHERE h.is_active = 1
            ) sub WHERE sub.distance_m <= :radius ORDER BY sub.distance_m
        ");
        $stmt->execute([':lat1' => (float)$c['latitude'], ':lng' => (float)$c['longitude'], ':lat2' => (float)$c['latitude'], ':radius' => (float)$c['radius']]);
        $households = $stmt->fetchAll();
        foreach ($households as &$h) { $h['latitude'] = (float)$h['latitude']; $h['longitude'] = (float)$h['longitude']; $h['distance_m'] = round((float)$h['distance_m'], 1); $h['marker_color'] = PovertyCalculator::markerColor($h['poverty_status']); }
        unset($h);
    ...
    case 'POST:create': {
        requireAuth();   // both roles can add centers
        $data = Validator::json();
        Validator::make($data, [
            'name' => 'required|string|maxlen:200', 'worship_type' => 'required|in:masjid,gereja,klenteng,pura,vihara',
            'address' => 'required|string', 'latitude' => 'required|latitude', 'longitude' => 'required|longitude',
            'radius' => 'integer|min:50|max:5000',
        ])->validate_or_fail();

        $pdo = Database::get();
        $pdo->prepare("INSERT INTO religious_centers (name, worship_type, address, latitude, longitude, radius, contact_person, contact_phone, notes) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([
                Validator::sanitizeString($data['name']), $data['worship_type'],
                Validator::sanitizeString($data['address']),
                (float)$data['latitude'], (float)$data['longitude'], (int)($data['radius'] ?? 300),
    ...
    case 'POST:update': {
        ...
        $pdo->prepare("UPDATE religious_centers SET name=?, worship_type=?, address=?, latitude=?, longitude=?, radius=?, contact_person=?, contact_phone=?, notes=? WHERE id=?")
            ->execute([
                Validator::sanitizeString($data['name']), $data['worship_type'],
                Validator::sanitizeString($data['address']),
                (float)$data['latitude'], (float)$data['longitude'], (int)($data['radius'] ?? 300),
    ...
    case 'POST:patch': {
        requireAuth();
        if (!$id) Response::error('ID is required.', 400);
        $data = Validator::json(); $fields = []; $params = [];
        if (isset($data['radius'])) { Validator::make($data, ['radius' => 'required|integer|min:50|max:5000'])->validate_or_fail(); $fields[] = 'radius = ?'; $params[] = (int)$data['radius']; }
        ...
        $params[] = $id;
        Database::get()->prepare('UPDATE religious_centers SET ' . implode(', ', $fields) . ' WHERE id=? AND is_active=1')->execute($params);
